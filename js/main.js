    ...
    $.getJSON('https://www.pivotaltracker.com/services/v5/projects/' + myproject + '/search',
        {'token': token,
            'query':
                ...
                ') includedone:true'},
        function (s) {
            panel_body.append('<div class="updated_stories">Updated: ' + s.stories.total_hits +
                " stories with " + s.stories.total_points_completed + 'pts done of ' +
                s.stories.total_points + ' worked on</div>');
            ...
            if (typeof(sync_function) != 'undefined') {
                sync_function();
            }
        });
}
...
$(document).on('click', '.refreshme', function () {
    get_member_stories($(this).parent().attr("id"));
});
$(document).on('click', '.removeme', function () {
    $(this).parent().parent().parent().remove();
});
function remove_inactive() {
    $(".panel-body").filter(function (i, j) {
        return $(j).find(".story_body").length == 0
